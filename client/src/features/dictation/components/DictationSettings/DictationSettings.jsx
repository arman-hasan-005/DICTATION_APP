...
import { SPEED_OPTIONS, REPEAT_OPTIONS, PAUSE_OPTIONS } from '../../../../constants/dictationSettings';
  ...
  return (
    <div className={[styles.wrapper, compact ? styles.compact : ''].filter(Boolean).join(' ')}>
      ...
      <div className={styles.grid}>
          ...
          <div className={styles.voiceToggle}>
            {GENDERS.map(g => (
              <button
                ...
                className={[styles.voiceBtn, settings.voice === g.value ? styles.active : ''].filter(Boolean).join(' ')}
          ...
          <div className={styles.accentGrid}>
            {ACCENTS.map(a => (
              <button
                ...
                className={[styles.accentBtn, settings.accent === a.value ? styles.active : ''].filter(Boolean).join(' ')}
        ...
        <div className={styles.voiceInfoCard}>
          <div className={styles.voiceInfoRow}>
            ...
            <div>
              ...
              <p className={styles.voiceInfoDesc}>Use stored audio files from the database. Falls back to browser voice if not available.</p>
            </div>
          </div>
          <div className={styles.voiceInfoRow}>
            ...
            <div>
              ...
              <p className={styles.voiceInfoDesc}>Uses Google Cloud TTS. Falls back to browser voice automatically.</p>
            </div>
          </div>
          <div className={styles.voiceInfoRow}>
            <span className={styles.voiceInfoIcon}>🌐</span>
            <div>
              ...
              <p className={[styles.voiceInfoDesc, !browserSupported ? styles.unsupported : ''].filter(Boolean).join(' ')}>
                {browserSupported
        ...
        {/* ── Speed ── */}
        <div className={styles.field}>
          <label className={styles.label}>⚡ Speed — <strong>{settings.speed}×</strong></label>
          <input
          ...
          <div className={styles.sliderLabels}>
            <span>0.5× Slow</span><span>1× Normal</span><span>1.5× Fast</span>
          </div>
        </div>
          ...
          <div className={styles.repeatGrid}>
            {REPEAT_OPTIONS.map(n => (
              <button key={n} type="button"
                className={[styles.repeatBtn, settings.repeatCount === n ? styles.active : ''].filter(Boolean).join(' ')}
        ...
        <div className={styles.field}>
          ...
          <select className={styles.select} value={settings.pauseDuration} onChange={e => set('pauseDuration', Number(e.target.value))}>
        ...
        <div className={styles.field}>
          <label className={styles.toggleLabel}>
            ...
            <div
              className={[styles.toggle, settings.autoAdvance ? styles.toggleOn : ''].filter(Boolean).join(' ')}
              onClick={() => set('autoAdvance', !settings.autoAdvance)}
              role="switch" aria-checked={settings.autoAdvance} tabIndex={0}
              onKeyDown={e => (e.key === 'Enter' || e.key === ' ') && set('autoAdvance', !settings.autoAdvance)}
            >
      ...
      {!compact && (
        <div className={styles.preview}>
          ...
          <p className={styles.previewText}>
            <strong>{genderData?.icon} {genderData?.label} · {accentData?.icon} {accentData?.label}</strong> voice at{' '}
            <strong>{settings.speed}×</strong> speed, repeated <strong>{settings.repeatCount}×</strong>,
            pausing <strong>{settings.pauseDuration}s</strong>.
